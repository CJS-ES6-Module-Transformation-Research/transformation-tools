...
var createImportDeclaration_1 = require("../../../abstract_representation/es_tree_stuff/createImportDeclaration");
var ImportType;
(function (ImportType) {
...
function transformImport(js) {
            ...
            if (node.type === "VariableDeclaration" && node.declarations.length > 0 && node.declarations[0].init) {
                if (node.declarations[0].init.type === "CallExpression") {
                    if (node.declarations[0].init.callee.type === "Identifier" && node.declarations[0].init.callee.name === "require") {
                        ...
                        switch (namePattern.type) {
                            case "Identifier":
                                tmpData_1 = { importString: is, importNames: [], iType: ImportType.defaultI };
                                tmpData_1.importNames.push(namePattern.name);
                                break;
                            case "ObjectPattern":
                                tmpData_1 = { importString: is, importNames: [], iType: ImportType.named };
                                namePattern.properties.forEach(function (e) {
                                    if (e.type === "Property") {
                                        var x = e.value;
                                        tmpData_1.importNames.push(e.value.name);
                                    }
                                    else {
                                        tmpData_1.importNames.push(e.argument.name);
                                    }
                                });
                                break;
                            default:
                        ...
                        var stmt = node.declarations[0].init.arguments[0].value;
                        tmpData_1.importString = stmt.toString();
                        data.push(tmpData_1);
                        nodes.add(node);
    ...
    data.map(createImportDeclaration_1.createAnImportDeclaration)
        .reverse()
        .forEach(function (e) { return js.addAnImport(e); });
}
