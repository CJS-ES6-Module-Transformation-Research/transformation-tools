...
var RequireStringTransformer = /** @class */ (function () {
    ...
    RequireStringTransformer.prototype.getTransformed = function (path) {
        ...
        if (path.charAt(0) !== '.' && path.charAt(0) !== '/') {
            ...
            || !isDir(absolute)) {
            if (isJS(absolute)) {
                computedPath = path_1.extname(absolute) !== "" ? absolute : absolute + index_1._JS;
            }
            else if (isJSON(absolute)) {
                computedPath = path_1.extname(absolute) !== "" ? absolute : absolute + index_1._JSON;
            }
            else {
        ...
        function isJS(path) {
            return path_1.extname(path) === index_1._JS
                || fs_1.existsSync(path + index_1._JS);
        }
        function isJSON(path) {
            return path_1.extname(path) === index_1._JSON || fs_1.existsSync(path + index_1._JSON);
        }
        function isDir(path) {
            try {
