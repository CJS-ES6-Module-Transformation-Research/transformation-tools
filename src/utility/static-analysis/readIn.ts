...
import {RequireCall, RequireDeclaration} from "../Require";
...
// require('chai').assert(Object.keys(id_to_ms).length <= Object.keys(ms_to_id).length,`` )
	...
	function exStmtRequireCall(exp: CallExpression, index: number): void {

	}

	function readRequireDecl(stmt: RequireDeclaration , index: number) : void {
