...
const _moduleAccess_net = require('net');
const _moduleAccess_events = require('events');
let net = require('net');
let EventEmitter = _moduleAccess_events.EventEmitter;
class Client extends EventEmitter {
    ...
    pub(topic, payload, cb) {
        ...
        if (this.connected) {
            this.client.write(Buffer.from('p' + JSON.stringify({
                t: topic,
                p: payload
            }) + this.eof));
            cb(topic, payload);
