...
const _moduleAccess_bl = require('bl');
const _moduleAccess_should = require('should');
const _moduleAccess____lib_MemoryFileSystem = require('../lib/MemoryFileSystem.js');
var bl = require('bl');
...
describe('directory', function () {
    ...
    it('should make and remove directories (linux style)', function () {
        ...
        fs.readdirSync('/test/').should.be.eql([
            'sub',
            'sub2'
        ]);
    ...
    it('should make and remove directories (windows style)', function () {
        ...
        fs.readdirSync('C:').should.be.eql([
            'test',
            'root-dir'
        ]);
        fs.readdirSync('C:/test/').should.be.eql([
            'sub',
            'sub2'
        ]);
...
describe('streams', function () {
    describe('writable streams', function () {
        ...
        it('should accept pipes', function (done) {
            var fs = new MemoryFileSystem();
            bl(new Buffer('Hello')).pipe(fs.createWriteStream('/file')).once('finish', function () {
                fs.readFileSync('/file', 'utf8').should.be.eql('Hello');
                done();
            });
        });
...
describe('pathToArray', function () {
    it('should split path to an array of parts', function () {
        var fs = new MemoryFileSystem();
        fs.pathToArray('/a/b/c').should.be.eql([
            'a',
            'b',
            'c'
        ]);
        fs.pathToArray('C:/a/b').should.be.eql([
            'C:',
            'a',
            'b'
        ]);
...
describe('os', function () {
    ...
    describe('unix', function () {
        ...
        it('should readdir directories', function () {
            fileSystem.readdirSync('/a').should.be.eql([
                'index',
                'dir'
            ]);
    ...
    describe('windows', function () {
        ...
        it('should readdir directories', function () {
            fileSystem.readdirSync('C:\\a').should.be.eql([
                'index',
                'dir'
            ]);
