...
predicate isBaseAccessPath(AccessPath ap) {
  not ap = MkAccessStep(_, _)
}
...
class AccessPath extends TAccessPath {
  ...
  AccessPath getBasePath() {
    exists(AccessPath b | this = MkAccessStep(b, _) and result = b.getBasePath())
    or
    isBaseAccessPath(this) and result = this
  }
